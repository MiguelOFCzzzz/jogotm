...
export default function Jogo2D() {
  ...
  useEffect(() => {
    ...
    // ── B2: variáveis Shadow (Cid Kagenou) ──
    ...
    const render = () => {
      ...
      if(teclas[' ']&&player.cooldownAtaque<=0){
        if(cl==='guerreiro'){
          ...
          // Shadow Slash é tratado no bloco B3 abaixo
        } else {
          const mwx=mouseRef.current.x+camera.x, mwy=mouseRef.current.y+camera.y;
      ...
      // ── B3: Shadow (Cid Kagenou) ──
      if (cl === 'shadow') {
        // Projétil Shadow Slash — Espaço
        if (teclas[' '] && shadowProjCooldown <= 0) {
          const mwx = mouseRef.current.x + camera.x;
          const mwy = mouseRef.current.y + camera.y;
          const ang = Math.atan2(mwy - (player.y + 40), mwx - (player.x + 40));
          player.direcaoRad = ang;
          ...
          socketRef.current?.emit('atacar', { tipo: 'shadowSlash', angulo: ang });
        }
        ...
        // Ebony Swirl — Z
        if (teclas['z'] && ebonyCooldown <= 0 && !ebonyAtivo) {
          ebonyAtivo = true; ebonyTimer = 0;
          ebonyCooldown = EBONY_CD_MAX;
          socketRef.current?.emit('habilidade_z', { tipo: 'ebony_swirl' });
        }
        if (ebonyAtivo) {
          ebonyTimer++;
          // Dano contínuo em volta do player durante toda a duração
          monstros.forEach(m => {
            const d = Math.hypot((m.x + m.size / 2) - (player.x + 40), (m.y + m.size / 2) - (player.y + 40));
            if (d < EBONY_AREA + m.size / 2) { m.hp -= 1.2; m.hitTimer = 2; }
          });
        ...
        if (teclas['x'] && atomicCooldown <= 0 && !atomicAtivo) {
          ...
          atomicParticulas = Array.from({ length: 80 }, () => { const a = Math.random() * Math.PI * 2; const v = 2 + Math.random() * 7; return { x: player.x + 40, y: player.y + 40, vx: Math.cos(a) * v, vy: Math.sin(a) * v, vida: 30 + Math.random() * 60, r: 2 + Math.random() * 5 }; });
          socketRef.current?.emit('habilidade_z', { tipo: 'i_am_atomic' });
        }
        if (atomicAtivo) {
          atomicTimer++;
          if (atomicTimer === 80) {
      ...
      if (cl === 'aizen') {
        ...
        if (redClickPendente && ilusaoCooldown <= 0) {
          const mx2 = redClickPendente.x, my2 = redClickPendente.y;
          redClickPendente = null;
          const ang2 = Math.atan2(my2 - (player.y + 40), mx2 - (player.x + 40));
          player.direcaoRad = ang2;
        ...
        // Kyōka Suigetsu — Z
        if (teclas['z'] && kyokaCooldown <= 0 && !kyokaAtivo) { kyokaAtivo = true; kyokaTimer = 0; kyokaCooldown = KYOKA_CD_MAX; socketRef.current?.emit('habilidade_z', { tipo: 'kyoka_suigetsu' }); }
        if (kyokaAtivo) {
          kyokaTimer++;
        ...
        if (teclas['x'] && hogyokuCooldown <= 0 && !hogyokuAtivo) {
          ...
          hogyokuParticulas = Array.from({ length: 80 }, () => { const a = Math.random() * Math.PI * 2; const v = 1.5 + Math.random() * 5; return { x: canvasRef.current!.width / 2, y: canvasRef.current!.height / 2, vx: Math.cos(a) * v, vy: Math.sin(a) * v, vida: 40 + Math.random() * 80, r: 1.5 + Math.random() * 4, cor: '#fbbf24' }; });
          socketRef.current?.emit('habilidade_z', { tipo: 'hogyoku_fusion' });
        }
        if (hogyokuAtivo) {
          hogyokuTimer++;
          if (hogyokuTimer === Math.floor(HOGYOKU_DUR * .45) && !hogyokuKilled) {
            monstros.forEach(m => explosoesArea.push({ x: m.x + m.size / 2, y: m.y + m.size / 2, timer: 0, maxTimer: 50 }));
            monstros.length = 0; hogyokuKilled = true;
          }
      ...
      Object.values(remotosRef.current).forEach(jr=>desenharJogadorRemoto(ctx,jr,{guerreiro:imgGuerreiro,mago:imgMago,sombrio:imgSombrio}));

      const danoSlimeNormal=0.4+ondaAtual*0.18;
      const danoSlimeBoss=1.5+ondaAtual*0.25;
      const danoFinalBoss=2.5+ondaAtual*0.3;
      const estaInvulneravel=(cl==='mago'&&ryoikiMagoAtivo)||(cl==='sombrio'&&invocAtivo)||(cl==='shadow'&&atomicAtivo)||(cl==='aizen'&&hogyokuAtivo);
      ...
      for(let i=projectiles.length-1;i>=0;i--){
        ...
        // ── B4: projétil shadow_slash ──
        } else if(p.tipo==='shadow_slash'){
          ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(p.angulo);
          for(let t=1;t<=6;t++){
            ctx.globalAlpha=.4*(1-t/6);
            const sg=ctx.createRadialGradient(-t*9,0,0,-t*9,0,8);
            sg.addColorStop(0,'#d946ef'); sg.addColorStop(1,'rgba(168,85,247,0)');
            ctx.fillStyle=sg; ctx.beginPath(); ctx.arc(-t*9,0,8,0,Math.PI*2); ctx.fill();
          }
          ctx.globalAlpha=1;
          const sg2=ctx.createRadialGradient(0,0,0,0,0,14);
          sg2.addColorStop(0,'#fff'); sg2.addColorStop(.3,'#e879f9'); sg2.addColorStop(.7,'#d946ef'); sg2.addColorStop(1,'rgba(168,85,247,0)');
          ctx.shadowColor='#d946ef'; ctx.shadowBlur=22; ctx.fillStyle=sg2;
          ctx.beginPath(); ctx.arc(0,0,14,0,Math.PI*2); ctx.fill(); ctx.shadowBlur=0; ctx.restore();
          let hitShadow=false;
          for(const m of monstros){if(Math.hypot(p.x-(m.x+m.size/2),p.y-(m.y+m.size/2))<m.size/2+14){m.hp-=p.dano;m.hitTimer=8;explosoesArea.push({x:p.x,y:p.y,timer:0,maxTimer:30});hitShadow=true;break;}}
          if(hitShadow) projectiles.splice(i,1);

        // ── B4: projétil ilusao (Aizen) ──
          ...
          for(const m of monstros){
            if(Math.hypot(p.x-(m.x+m.size/2),p.y-(m.y+m.size/2))<m.size/2+12){
              ...
              if(kyokaAtivo){monstros.forEach(alv=>{if(alv!==m&&Math.hypot((alv.x+alv.size/2)-(m.x+m.size/2),(alv.y+alv.size/2)-(m.y+m.size/2))<120){alv.hp-=p.dano*.5;alv.hitTimer=6;}});}
              hitIlusao=true; break;
            }
          }
          if(hitIlusao) projectiles.splice(i,1);
      ...
      // ── B5: desenhos de habilidades Shadow ──
      if (cl === 'shadow') {
      ...
      if(cl==='sombrio'){
        ...
        if(sombrasFila.length>0){
          ...
          filaOrdenada.slice(0,6).forEach((s,i)=>{
            ...
            ctx.fillText(`${i===0?'▶':' '} ${iconeTipo(s.tipo)} ${s.tipo==='mago_ini'?'MAGO':s.tipo.toUpperCase()}`,filaX,fy);
          });
      ...
      if (cl === 'shadow') {
        ...
        const bx = canvas.width - 74, by = canvas.height - 74, bs = 58;
        ctx.fillStyle = eAt ? 'rgba(168,85,247,.97)' : ePr ? 'rgba(124,58,237,.92)' : 'rgba(15,5,30,.88)'; ctx.fillRect(bx, by, bs, bs);
        ctx.strokeStyle = eAt ? '#d946ef' : ePr ? '#a855f7' : '#4c1d95'; ctx.lineWidth = eAt || ePr ? 2.5 : 1.5;
        if (eAt || ePr) { ctx.shadowColor = '#d946ef'; ctx.shadowBlur = eAt ? 22 : 12; }
        ctx.strokeRect(bx, by, bs, bs); ctx.shadowBlur = 0;
        if (!ePr && !eAt) { ctx.fillStyle = 'rgba(0,0,0,.55)'; ctx.fillRect(bx, by, bs, bs * eCdr); }
        ctx.font = 'bold 22px sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = eAt ? '#f0abfc' : ePr ? '#d946ef' : '#4c1d95'; ctx.fillText('🌀', bx + bs / 2, by + 34);
        ctx.font = 'bold 9px monospace'; ctx.fillStyle = eAt ? '#faf5ff' : ePr ? '#e9d5ff' : '#4c1d95'; ctx.fillText('[Z] EBONY', bx + bs / 2, by + 52);
        ...
        ctx.fillStyle = ssPr ? 'rgba(124,58,237,.90)' : 'rgba(15,5,30,.88)'; ctx.fillRect(ssBx, ssBy, ssBs, ssBs);
        ctx.strokeStyle = ssPr ? '#d946ef' : '#4c1d95'; ctx.lineWidth = ssPr ? 2.5 : 1.5;
        if (ssPr) { ctx.shadowColor = '#d946ef'; ctx.shadowBlur = 10; }
        ctx.strokeRect(ssBx, ssBy, ssBs, ssBs); ctx.shadowBlur = 0;
        if (!ssPr) { ctx.fillStyle = 'rgba(0,0,0,.55)'; ctx.fillRect(ssBx, ssBy, ssBs, ssBs * (shadowProjCooldown / SHADOW_PROJ_CD)); }
        ctx.font = 'bold 22px sans-serif'; ctx.fillStyle = ssPr ? '#f0abfc' : '#4c1d95'; ctx.fillText('⚔', ssBx + ssBs / 2, ssBy + 34);
        ctx.font = 'bold 9px monospace'; ctx.fillStyle = ssPr ? '#e9d5ff' : '#4c1d95'; ctx.fillText('[ESPAÇO]', ssBx + ssBs / 2, ssBy + 52);
        ...
        ctx.fillStyle = aaAt ? 'rgba(168,85,247,.99)' : aaPr ? 'rgba(124,58,237,.95)' : 'rgba(15,5,30,.9)'; ctx.fillRect(ubx, uby, ubs, ubs);
        ctx.strokeStyle = aaAt ? '#f0abfc' : aaPr ? '#d946ef' : '#4c1d95'; ctx.lineWidth = aaAt || aaPr ? 2.5 : 1.5;
        if (aaAt || aaPr) { ctx.shadowColor = aaAt ? '#f0abfc' : '#d946ef'; ctx.shadowBlur = aaAt ? 28 : 14; }
        ctx.strokeRect(ubx, uby, ubs, ubs); ctx.shadowBlur = 0;
        if (!aaPr && !aaAt) { ctx.fillStyle = 'rgba(0,0,0,.55)'; ctx.fillRect(ubx, uby, ubs, ubs * aaCdr); }
        ctx.font = 'bold 7px monospace'; ctx.fillStyle = aaAt ? '#faf5ff' : aaPr ? '#e9d5ff' : '#4c1d95'; ctx.fillText('ULTIMATE', ubx + ubs / 2, uby + 11);
        ctx.font = 'bold 11px monospace'; ctx.fillStyle = aaAt ? '#f0abfc' : aaPr ? '#d946ef' : '#4c1d95';
        if (aaAt || aaPr) { ctx.shadowColor = '#d946ef'; ctx.shadowBlur = aaAt ? 14 : 8; }
        ctx.fillText('ATOMIC', ubx + ubs / 2, uby + 30); ctx.shadowBlur = 0;
        ctx.font = 'bold 10px monospace'; ctx.fillStyle = aaAt ? '#faf5ff' : aaPr ? '#e9d5ff' : '#4c1d95'; ctx.fillText('[X]', ubx + ubs / 2, uby + 50);
      ...
      if (cl === 'aizen') {
        ...
        ctx.fillStyle = ilPr ? 'rgba(120,80,0,.92)' : 'rgba(20,12,0,.88)'; ctx.fillRect(ilBx, ilBy, ilBs, ilBs);
        ctx.strokeStyle = ilPr ? '#fbbf24' : '#78350f'; ctx.lineWidth = ilPr ? 2.5 : 1.5;
        if (ilPr) { ctx.shadowColor = '#fbbf24'; ctx.shadowBlur = 12; }
        ctx.strokeRect(ilBx, ilBy, ilBs, ilBs); ctx.shadowBlur = 0;
        if (!ilPr) { ctx.fillStyle = 'rgba(0,0,0,.55)'; ctx.fillRect(ilBx, ilBy, ilBs, ilBs * (ilusaoCooldown / ILUSAO_CD)); }
        ctx.font = 'bold 22px sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = ilPr ? '#fde68a' : '#78350f'; ctx.fillText('🪞', ilBx + ilBs / 2, ilBy + 34);
        ctx.font = 'bold 9px monospace'; ctx.fillStyle = ilPr ? '#fef9c3' : '#78350f'; ctx.fillText('[CLICK]', ilBx + ilBs / 2, ilBy + 52);
        ...
        ctx.fillStyle = kAt ? 'rgba(120,80,0,.97)' : kPr ? 'rgba(100,65,0,.92)' : 'rgba(18,10,0,.88)'; ctx.fillRect(kbx, kby, kbs, kbs);
        ctx.strokeStyle = kAt ? '#fde68a' : kPr ? '#fbbf24' : '#78350f'; ctx.lineWidth = kAt || kPr ? 2.5 : 1.5;
        if (kAt || kPr) { ctx.shadowColor = '#fbbf24'; ctx.shadowBlur = kAt ? 20 : 12; }
        ctx.strokeRect(kbx, kby, kbs, kbs); ctx.shadowBlur = 0;
        ...
        ctx.font = 'bold 10px monospace'; ctx.fillStyle = kAt ? '#fef9c3' : kPr ? '#fde68a' : '#78350f'; ctx.fillText('KYOKA', kbx + kbs / 2, kby + 30);
        ...
        ctx.fillStyle = hAt ? 'rgba(80,10,120,.99)' : hPr ? 'rgba(60,5,90,.95)' : 'rgba(15,0,25,.9)'; ctx.fillRect(hbx, hby, hbs, hbs);
        ctx.strokeStyle = hAt ? '#fbbf24' : hPr ? '#a855f7' : '#4c1d95'; ctx.lineWidth = hAt || hPr ? 2.5 : 1.5;
        if (hAt || hPr) { ctx.shadowColor = hAt ? '#fbbf24' : '#a855f7'; ctx.shadowBlur = hAt ? 28 : 14; }
        ctx.strokeRect(hbx, hby, hbs, hbs); ctx.shadowBlur = 0;
        if (!hPr && !hAt) { ctx.fillStyle = 'rgba(0,0,0,.55)'; ctx.fillRect(hbx, hby, hbs, hbs * hCdr); }
        ctx.font = 'bold 7px monospace'; ctx.fillStyle = hAt ? '#fde68a' : hPr ? '#c084fc' : '#4c1d95'; ctx.fillText('ULTIMATE', hbx + hbs / 2, hby + 11);
        ctx.font = 'bold 12px monospace'; ctx.fillStyle = hAt ? '#fde68a' : hPr ? '#fbbf24' : '#4c1d95';
        if (hAt || hPr) { ctx.shadowColor = hAt ? '#fbbf24' : '#a855f7'; ctx.shadowBlur = hAt ? 16 : 8; }
        ctx.fillText('宝玉', hbx + hbs / 2, hby + 30); ctx.shadowBlur = 0;
        ctx.font = 'bold 10px monospace'; ctx.fillStyle = hAt ? '#fde68a' : hPr ? '#f5d0fe' : '#4c1d95'; ctx.fillText('[X]', hbx + hbs / 2, hby + 50);
  ...
  }, [status, imgPlayer, imgArmaGuerr, imgArmaMago, imgCenario, imgGuerreiro, imgMago, imgSombrio, imgShadow, imgAizen]);
    ...
    <main className="relative min-h-screen bg-black flex flex-col items-center justify-center font-serif text-white overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <p suppressHydrationWarning className="mt-4 text-zinc-500 text-[10px] text-center uppercase tracking-tighter">
          ...
          {status.classe==='aizen'     && 'CLIQUE = Ilusão · Z = Kyōka Suigetsu · X = Hōgyoku Fusion [ultimate] · '}
